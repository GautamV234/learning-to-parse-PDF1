...
class CustomVisionTransformer(VisionTransformer):
    ...
    def forward_features(self, x):
        ...
        # print(self.width//self.patch_size-w)
        # print(torch.arange(h)*(self.width//self.patch_size-w))
        ...
        # print(self.pos_embed.shape3
        # print(pos_emb_ind.shape)

        x += self.pos_embed[:, pos_emb_ind]
        # try:
        # except:
        #     pos_emb_ind = repeat(torch.arange(h)*(self.width//self.patch_size-w), 'h -> (h w)', w=w)+torch.arange(h*w)
        #     pos_emb_ind = torch.cat((torch.zeros(3), pos_emb_ind+1), dim=0).long()
        #     x += self.pos_embed[:, pos_emb_ind]
...
def get_model(args, training=False):
    ...
    if training:
        ...
        im = torch.empty(args.batchsize, args.channels, args.min_width, args.min_height, device=args.device).float()
