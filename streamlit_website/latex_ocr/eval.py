...
def evaluate(model: Model, dataset: Im2LatexDataset, args: Munch, num_batches: int = None, name: str = 'test'):
    ...
    for i, (seq, im) in pbar:
        ...
        # print("".join(pred[0]), "".join(truth[0]))
        CER = fastwer.score(["".join(pred[0])], ["".join(truth[0])], char_level=True)
        cer.append(CER)
        pbar.set_description('BLEU: %.3f, ED: %.2e, cer: %.2e' % (np.mean(bleus), np.mean(edit_dists), np.mean(cer)))
